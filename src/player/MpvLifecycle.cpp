...
MpvLifecycle::~MpvLifecycle() { destroy(); }

mpv_handle *MpvLifecycle::handle() const { return m_handle.load(); }
...
bool MpvLifecycle::adopt(mpv_handle *handle, EventHandler eventHandler) {
  ...
  m_eventThread = std::thread(
      [this, handle, eventHandler = std::move(eventHandler)]() mutable {
...
void MpvLifecycle::requestEventLoopStop() { m_terminating = true; }
...
void MpvLifecycle::cancelFileLoad() {
  ...
  while (pending > 0 && !m_pendingFileLoads.compare_exchange_weak(
                            pending, pending - 1, std::memory_order_acq_rel,
                            std::memory_order_acquire)) {
  }
}

void MpvLifecycle::completeFileLoad() { cancelFileLoad(); }
