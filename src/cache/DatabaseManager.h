...
public:
    ...
    int schemaVersion();
    QByteArray loadCacheEntry(const QString& nameSpace, const QString& key, qint64 maxAgeMs = -1);
...
private:
    template <typename Callback> auto invokeOnWorker(Callback&& callback) -> std::invoke_result_t<Callback>
    {
        using Result = std::invoke_result_t<Callback>;
        Result result {};
        QMetaObject::invokeMethod(
            m_worker, [&]() { result = std::invoke(std::forward<Callback>(callback)); }, Qt::BlockingQueuedConnection);
        return result;
    }
