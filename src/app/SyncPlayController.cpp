...
SyncPlayController::SyncPlayController(JellyfinApiFacade *api,
    ...
    , m_player(player)
{
    ...
    connect(&m_socket, &QWebSocket::errorOccurred, this,
            [this](QAbstractSocket::SocketError) {
                emit errorText(QStringLiteral("SyncPlay socket error: %1")
                                   .arg(m_socket.errorString()));
            });
    connect(&m_timeSyncTimer, &QTimer::timeout, this,
            &SyncPlayController::requestTimeSync);
    connect(&m_commandTimer, &QTimer::timeout, this,
            &SyncPlayController::executeScheduledCommand);
    connect(&m_correctionTimer, &QTimer::timeout, this,
            &SyncPlayController::correctPlaybackDrift);
    connect(&m_reconnectTimer, &QTimer::timeout, this,
            &SyncPlayController::connectSocket);
    if (m_player) {
        connect(m_player, &PlayerController::stateChanged, this,
                &SyncPlayController::handlePlayerStateChanged);
    }
}
...
void SyncPlayController::refreshGroups()
{
    ...
    Async::runScoped(
        ...
        [this](const QJsonArray &groups) {
            ...
            if (!m_groupId.isEmpty()) {
                for (const QJsonValue &value : groups) {
                    ...
                    if (group.value(QStringLiteral("GroupId")).toString() ==
                        m_groupId) {
        ...
        [this](const std::exception_ptr &error) {
            reportRequestError(QStringLiteral("refresh groups"), error);
        });
}
...
void SyncPlayController::connectSocket()
{
    ...
    request.setRawHeader("X-Emby-Token",
                         m_api->session().accessToken.toUtf8());
...
void SyncPlayController::createGroup(const QString &name)
{
    ...
    connectSocket();
    Async::runScoped(
        this, m_api->createSyncPlayGroup(name),
        [this]() { refreshGroups(); },
        [this](const std::exception_ptr &error) {
            reportRequestError(QStringLiteral("create group"), error);
        });
}
    ...
    Async::runScoped(
        ...
        [this, groupId]() {
            ...
            m_joinedAtServerMs =
                ...
                static_cast<qint64>(std::llround(m_clock.offsetMs()));
            emit groupChanged();
            sendPlayerBufferingState(true);
        },
        [this](const std::exception_ptr &error) {
            reportRequestError(QStringLiteral("join group"), error);
        });
}
...
void SyncPlayController::leaveGroup()
{
    if (!m_api || m_groupId.isEmpty())
        return;
    Async::runScoped(
        this, m_api->leaveSyncPlayGroup(),
        [this]() { clearGroup(); },
        [this](const std::exception_ptr &error) {
            reportRequestError(QStringLiteral("leave group"), error);
        });
}
...
void SyncPlayController::requestPause()
{
    ...
    Async::runScoped(
        ...
        [this](const std::exception_ptr &error) {
            reportRequestError(QStringLiteral("pause"), error);
        },
...
void SyncPlayController::requestPlay()
{
    ...
    Async::runScoped(
        ...
        [this](const std::exception_ptr &error) {
            reportRequestError(QStringLiteral("play"), error);
        },
...
void SyncPlayController::requestSeekSeconds(double seconds)
{
    if (!m_api || m_groupId.isEmpty())
        return;
    const qint64 ticks =
        static_cast<qint64>(std::max(0.0, seconds) * kTicksPerSecond);
    Async::runScoped(
        this, m_api->syncPlayRequestSeek(ticks), []() {},
        [this](const std::exception_ptr &error) {
            reportRequestError(QStringLiteral("seek"), error);
        },
...
void SyncPlayController::handleSocketTextMessage(const QString &message)
{
    ...
    if (!document.isObject()) {
        qWarning() << "syncplay: invalid websocket message"
...
void SyncPlayController::handleSyncPlayCommand(const QJsonObject &data)
{
    ...
    const QString groupId =
        data.value(QStringLiteral("GroupId")).toString();
    if (!groupId.isEmpty() && groupId != m_groupId)
        return;

    const qint64 emittedAtMs =
        dateTimeMs(data.value(QStringLiteral("EmittedAt")));
    if (emittedAtMs > 0 && m_joinedAtServerMs > 0 &&
        emittedAtMs < m_joinedAtServerMs) {
        qInfo() << "syncplay: ignoring command emitted before group join";
        return;
    }

    const qint64 serverTimeMs =
        dateTimeMs(data.value(QStringLiteral("When")));
    const qint64 positionTicks =
        data.value(QStringLiteral("PositionTicks")).toVariant().toLongLong();
    const QString playlistItemId =
        data.value(QStringLiteral("PlaylistItemId")).toString();
    ...
    const qint64 delayMs =
        serverTimeMs > 0
            ? (m_clock.ready()
                   ? m_clock.localDelayUntil(serverTimeMs, localNowMs)
                   : std::max<qint64>(0, serverTimeMs - localNowMs))
            : 0;
    m_commandTimer.start(static_cast<int>(
        std::min<qint64>(delayMs, std::numeric_limits<int>::max())));
}

void SyncPlayController::handleSyncPlayGroupUpdate(
    const QJsonObject &data)
{
    ...
    if (type == QStringLiteral("GroupJoined") ||
        type == QStringLiteral("GroupUpdate")) {
    ...
    if (type == QStringLiteral("UserJoined")) {
        ...
        if (!participant.isEmpty() &&
            !m_participants.contains(participant)) {
    ...
    if (type == QStringLiteral("StateUpdate")) {
        m_groupState =
            updateData.toObject().value(QStringLiteral("State")).toString();
        emit groupChanged();
        return;
    }
    if (type == QStringLiteral("PlayQueue")) {
        const QJsonObject queue = updateData.toObject();
        const QJsonArray playlist =
            queue.value(QStringLiteral("Playlist")).toArray();
        const int playingIndex =
            queue.value(QStringLiteral("PlayingItemIndex")).toInt(-1);
        if (playingIndex >= 0 && playingIndex < playlist.size()) {
            m_playlistItemId =
                playlist.at(playingIndex)
                    .toObject()
    ...
    if (type == QStringLiteral("GroupLeft") ||
        type == QStringLiteral("NotInGroup")) {
        clearGroup();
        return;
    }
    if (type == QStringLiteral("GroupDoesNotExist") ||
        type == QStringLiteral("LibraryAccessDenied") ||
        type == QStringLiteral("CreateGroupDenied") ||
        type == QStringLiteral("JoinGroupDenied") ||
...
void SyncPlayController::applyGroupInfo(const QString &groupId,
                                        const QJsonObject &info)
{
    const QString infoGroupId =
        info.value(QStringLiteral("GroupId")).toString();
    ...
    for (const QJsonValue &value :
         info.value(QStringLiteral("Participants")).toArray()) {
...
void SyncPlayController::executeScheduledCommand()
{
    ...
    if (command == QStringLiteral("Unpause") &&
        ...
        positionTicks = m_clock.estimatePositionTicks(
            positionTicks, m_scheduledServerTimeMs,
            QDateTime::currentMSecsSinceEpoch());
    }
    const double targetSeconds =
        static_cast<double>(positionTicks) / kTicksPerSecond;
    const double positionDelta =
        std::abs(m_player->positionSeconds() - targetSeconds);
    ...
    if (command == QStringLiteral("Pause")) {
        ...
        if (positionDelta * 1'000.0 > kDriftCorrectionThresholdMs &&
            !m_player->seeking()) {
            m_player->seek(targetSeconds);
        }
        if (m_player->paused())
        ...
        QTimer::singleShot(250, this,
                           [this]() { sendPlayerBufferingState(true); });
    } else if (command == QStringLiteral("Stop")) {
        m_player->stopWithReason(QStringLiteral("syncplay-stop"));
    } else {
        emit errorText(
            QStringLiteral("Unsupported SyncPlay command: %1").arg(command));
    }
}

void SyncPlayController::correctPlaybackDrift()
{
    if (!enabled() || !m_player || !m_player->visible() ||
        m_scheduledCommand != QStringLiteral("Unpause") ||
        m_scheduledServerTimeMs <= 0 || m_player->paused() ||
        m_player->buffering() || m_player->seeking()) {
        return;
    }
    ...
    const qint64 expectedTicks = m_clock.estimatePositionTicks(
        m_scheduledPositionTicks, m_scheduledServerTimeMs, nowMs);
    const qint64 actualTicks = static_cast<qint64>(
        m_player->positionSeconds() * kTicksPerSecond);
    const qint64 diffMs =
        std::abs(expectedTicks - actualTicks) / kTicksPerMillisecond;
...
void SyncPlayController::sendPlayerBufferingState(bool force)
{
    ...
    if (!force && m_playerStateKnown &&
        buffering == m_lastPlayerBuffering) {
        return;
    }
    m_playerStateKnown = true;
    m_lastPlayerBuffering = buffering;

    const qint64 positionTicks = static_cast<qint64>(
        m_player->positionSeconds() * kTicksPerSecond);
    ...
    Async::runScoped(
        this,
        m_api->syncPlayReportBuffering(
            buffering, positionTicks, playing, m_playlistItemId,
            QDateTime::fromMSecsSinceEpoch(
                serverNowMs, QTimeZone(QTimeZone::UTC))),
        []() {},
        [this, buffering](const std::exception_ptr &error) {
            reportRequestError(
                buffering ? QStringLiteral("report buffering")
                          : QStringLiteral("report ready"),
                error);
        },
...
void SyncPlayController::requestTimeSync()
{
    ...
    Async::runScoped(
        ...
        [this, requestSentMs](const QJsonObject &response) {
            m_timeSyncInFlight = false;
            const qint64 responseReceivedMs =
                QDateTime::currentMSecsSinceEpoch();
            const qint64 requestReceivedMs = dateTimeMs(
                response.value(QStringLiteral("RequestReceptionTime")));
            const qint64 responseSentMs = dateTimeMs(
                response.value(QStringLiteral("ResponseTransmissionTime")));
            if (requestReceivedMs <= 0 || responseSentMs <= 0) {
                if (!m_timeSyncErrorReported) {
                    ...
                    emit errorText(QStringLiteral(
                        "SyncPlay time response was incomplete."));
                }
                scheduleTimeSync();
                return;
            }

            m_clock.addMeasurement({requestSentMs, requestReceivedMs,
                                    responseSentMs, responseReceivedMs});
            m_timeSyncErrorReported = false;
            emit connectionChanged();

            Async::runScoped(
                this,
                m_api->syncPlayReportPing(static_cast<qint64>(
                    std::llround(m_clock.pingMs()))),
                []() {},
                [this](const std::exception_ptr &error) {
                    reportRequestError(QStringLiteral("report ping"), error);
                },
        ...
        [this](const std::exception_ptr &error) {
            ...
            if (!m_timeSyncErrorReported) {
                m_timeSyncErrorReported = true;
                reportRequestError(QStringLiteral("synchronize clock"),
                                   error);
            }
            scheduleTimeSync();
        },
...
void SyncPlayController::scheduleTimeSync()
{
    if (!socketConnected())
        return;
    const int interval = m_greedyTimeSyncRemaining > 0
                             ? kGreedyTimeSyncIntervalMs
                             : kSteadyTimeSyncIntervalMs;
    m_greedyTimeSyncRemaining =
        std::max(0, m_greedyTimeSyncRemaining - 1);
    m_timeSyncTimer.start(interval);
}

void SyncPlayController::reportRequestError(
    const QString &action, const std::exception_ptr &error)
{
    const QString message = exceptionMessage(error);
    qWarning() << "syncplay:" << action << "failed:" << message;
    emit errorText(
        QStringLiteral("SyncPlay %1 failed: %2").arg(action, message));
}
...
void SyncPlayController::sendKeepAlive()
{
    if (socketConnected()) {
        m_socket.sendTextMessage(
            QStringLiteral(R"({"MessageType":"KeepAlive"})"));
    }
}
...
QUrl SyncPlayController::socketUrl() const
{
    ...
    QUrlQuery query;
    query.addQueryItem(QStringLiteral("api_key"),
                       m_api->session().accessToken);
    if (!m_api->deviceId().isEmpty()) {
        query.addQueryItem(QStringLiteral("deviceId"),
                           m_api->deviceId());
    }
    url.setQuery(query);
    return url;
