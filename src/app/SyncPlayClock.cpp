...
void SyncPlayClock::addMeasurement(const SyncPlayTimeMeasurement& measurement)
{
...
std::int64_t SyncPlayClock::localDelayUntil(std::int64_t serverTimeMs, std::int64_t localNowMs) const
{
...
void SyncPlayClock::selectBestMeasurement()
{
    ...
    m_best = *std::min_element(m_measurements.cbegin(), m_measurements.cend(),
        [](const SyncPlayTimeMeasurement& left, const SyncPlayTimeMeasurement& right) {
