...
QuickConnectController::QuickConnectController(JellyfinApiFacade *api,
                                               QObject *parent)
...
QString QuickConnectController::code() const { return m_code; }

QString QuickConnectController::status() const { return m_status; }

bool QuickConnectController::active() const { return !m_secret.isEmpty(); }
  ...
  Async::runScoped(
      ...
      [this, generation](bool enabled) {
        ...
        if (!enabled) {
          ...
          emit errorOccurred(
              QStringLiteral("Quick Connect is disabled on this server."));
          return;
        }
  ...
  Async::runScoped(
      ...
      [this, secret](const QJsonObject &result) {
        ...
        const bool isAuthenticated =
            result.value(QStringLiteral("Authenticated")).toBool();
        qInfo() << "quick connect: poll" << m_pollAttempts << "authenticated"
                << isAuthenticated;
        if (!isAuthenticated)
          return;
