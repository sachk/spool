...
- ~~**Image-prefetch hard-capped at 24 posters with magic numbers** — `AppController.cpp:1205` truncates to 24, concurrency `6` is a literal, while lists fetch up to 500 (`JellyfinApiFacade.cpp:335`). Track the visible viewport instead; lift caps to config. **M**~~ **Done in the 2026-07 refactor:** visible-range prefetch is owned by `BrowseSessionController`/`LibraryPrefetchController`, and ahead/concurrency are settings-backed.
...
- ~~**Search & ItemDetails reuse `appController.movies`, producing wrong content** — `qml/pages/SearchPage.qml:78` filters the movie grid client-side by title substring (`:89`); `ItemDetailsPage.qml:384` "More Like This" re-lists the entire movies model. Both need dedicated controller models. **M**~~ **Done in the 2026-07 refactor:** `SearchController` and `ContentModelController` own dedicated result/context models.
- **Duplicated accept-key / navigation handling remains reduced but not gone** — shared `InputKeys`, `NavGrid`, `MenuListView`, and extracted popups removed the largest duplicate state machines, but page-local actions still call `InputKeys.isAccept(...)` at intentional activation sites. Keep this as a smaller follow-up, not the original broad refactor.
- ~~**Two near-identical horizontal-row components in HomePage** — `qml/pages/HomePage.qml:267` (`rowComponent`) and `:404` (`libraryRowComponent`) share `ensureVisible()`/`handleNavigationKey()`/Flickable+Repeater scaffolding. Collapse into one parameterized row. **M**~~ **Done in the 2026-07 refactor:** `HomeHorizontalRow` covers library, resume, next-up, and latest rows.
- ~~**HomePage repeats the same 6-handler Connections block 4×** — `qml/pages/HomePage.qml:21` (latest/library/resume/nextUp), each re-caching `rowCount()` on model changes. Drive off the models directly. **S**~~ **Done in the 2026-07 refactor:** Home rows bind to controller-owned models and one `Home.latestLibraryRows` connection.
...
- ~~**Dead settings UI** — `qml/pages/SettingsPage.qml:437` (max remote bitrate), `:449` (prefer remux), `:467` (shortcuts enabled) have no handler / hardcoded `checked`. Wire to `appController` or remove. **M**~~ **Done in the 2026-07 refactor:** settings are generated from `SettingsSchema` and persisted/applied through `SettingsController`.
- ~~**Selectors don't reflect persisted state** — `SettingsPage.qml:340` — several `SelectRow`s set literal `currentIndex` so the shown selection desyncs from the live singleton after restart. Bind `currentIndex`. **S**~~ **Done in the 2026-07 refactor:** `SettingsController::values` drives schema rows from persisted normalized values.
