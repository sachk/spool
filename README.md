...
- Qt6.11
  ...
  Async layer: upstream QCoro on top of the Qt async APIs, so network/database orchestration stays coroutine-based. Native builds use the nixpkgs QCoro 0.12 package. The webOS cross-build temporarily uses `third_party/qcoro` through `JELLYFIN_USE_BUNDLED_QCORO=ON` until a target-built QCoro package is added.
