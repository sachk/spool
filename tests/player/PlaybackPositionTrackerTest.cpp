...
int main()
{
    ...
    require(near(tracker.seekAnchor(true, false), 10.0), "seek target was not used as the anchor");
    ...
    require(near(tracker.position(), 80.0), "stale forward-seek sample replaced optimistic target");
    ...
    require(near(tracker.position(), 20.0), "stale backward-seek sample replaced optimistic target");
