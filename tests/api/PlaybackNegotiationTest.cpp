...
QJsonObject source(const QString &id, qint64 bitrate, bool directPlay,
                   bool directStream, const QString &directStreamUrl = {},
...
int main(int argc, char **argv)
{
    ...
    const PlaybackSelection direct =
        PlaybackNegotiation::selectSource(sources, true);
    require(direct.playMethod == QStringLiteral("DirectPlay"),
            "direct play should outrank fallback methods");
    require(direct.source.value(QStringLiteral("Id")).toString() ==
                QStringLiteral("direct-high"),
    ...
    const PlaybackSelection remux =
        PlaybackNegotiation::selectSource(fallbackSources, true);
    require(remux.playMethod == QStringLiteral("DirectStream"),
            "remux preference should select direct stream");
    const PlaybackSelection transcode =
        PlaybackNegotiation::selectSource(fallbackSources, false);
    require(transcode.playMethod == QStringLiteral("Transcode"),
            "disabling remux preference should select transcode");

    const QString url = PlaybackNegotiation::buildUrl(
        QStringLiteral("https://example.test/jellyfin"), QStringLiteral("item"),
        QStringLiteral("secret"), transcode);
    ...
    require(QUrlQuery(parsed).queryItemValue(QStringLiteral("api_key")) ==
                QStringLiteral("secret"),
            "playback URLs should include the access token");

    const QJsonObject profile =
        PlaybackNegotiation::buildDeviceProfile(25'000'000);
    require(profile.value(QStringLiteral("MaxStreamingBitrate")).toInteger() ==
                25'000'000,
            "device profile should carry the configured bitrate");
    require(!profile.value(QStringLiteral("TranscodingProfiles"))
                 .toArray()
                 .isEmpty(),
